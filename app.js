...
function saveServerCache(nodesList, metricsMap) {
  const metricsObj = Object.fromEntries(Array.from(metricsMap.entries()));
  const payload = {
    timestamp: Date.now(),
    nodes: nodesList,
    metrics: metricsObj,
...
function applyUserFiltersFromStorage(filters) {
  ...
  if (cpuMaxFilter) cpuMaxFilter.value = filters.cpuMax ?? "";
  if (ioMaxFilter) ioMaxFilter.value = filters.ioMax ?? "";
  if (nicMaxFilter) nicMaxFilter.value = filters.nicMax ?? "";
  if (congestionMaxFilter) congestionMaxFilter.value = filters.congestionMax ?? "";

  selectedCountryCodes.clear();
  if (Array.isArray(filters.countryCodes)) {
    filters.countryCodes.forEach((code) => selectedCountryCodes.add(code));
  }
  refreshCountryFilterUI();

  if (filters.sort && filters.sort.key) {
    sortState = { key: filters.sort.key, direction: filters.sort.direction === "desc" ? "desc" : "asc" };
  }
